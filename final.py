...
def transaction_history(user_id):
    transactions = cursor.execute("SELECT amount, recipient_account, timestamp FROM transactions WHERE user_id = ? ORDER BY timestamp DESC", (user_id,)).fetchall()
    print("************** Transaction History **************")
    if not transactions:
        print("No transactions found.")
    else:
        for transaction in transactions:
            recipient = transaction[1] if transaction[1] else "Self"
            print(f"{transaction} - Amount: {transaction[0]:.2f} to {recipient}")
